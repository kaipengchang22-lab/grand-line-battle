...
const M={skin:mat('Warm skin','#c98561',.78),skin2:mat('Skin light','#eda985',.78),red:mat('Crimson vest','#9d2b27',.6),blue:mat('Indigo shorts','#183f81',.72),yellow:mat('Straw gold','#d79e29',.72),straw:mat('Woven straw','#c7963e',.8),hair:mat('Black hair','#20151a',.85),white:mat('Eye white','#f7f3ed',.35),black:mat('Pupil','#181722',.35),brown:mat('Sandal leather','#42251d',.75),scar:mat('Scar','#9f5245',.9)};
...
const head=node('Head',torso,Q.sphere_skin2,[0,.92,0],[.48,.50,.43]);
node('Ear_L',head,Q.sphere_skin2,[-.47,.0,0],[.11,.15,.10]);node('Ear_R',head,Q.sphere_skin2,[.47,.0,0],[.11,.15,.10]);
node('Eye_L',head,Q.sphere_white,[-.16,.05,-.39],[.105,.13,.035]);node('Eye_R',head,Q.sphere_white,[.16,.05,-.39],[.105,.13,.035]);
node('Pupil_L',head,Q.sphere_black,[-.16,.04,-.43],[.043,.06,.022]);node('Pupil_R',head,Q.sphere_black,[.16,.04,-.43],[.043,.06,.022]);
node('Scar',head,Q.cube_scar,[.29,-.13,-.43],[.018,.11,.018],[0,0,.45]);
node('Hat_Brim',head,Q.cyl_straw,[0,.43,0],[.67,.08,.67]);node('Hat_Crown',head,Q.cyl_straw,[0,.54,.0],[.43,.20,.43]);node('Hat_Band',head,Q.cyl_red,[0,.51,0],[.445,.045,.445]);
for(let i=0;i<7;i++){const a=(i-3)*.17;node('Hair_'+i,head,Q.sphere_hair,[a,.30,-.34],[.11,.20,.10],[0,0,a*.7]);}
function limb(side){const sx=side==='L'?-1:1;const arm=node('UpperArm_'+side,torso,undefined,[sx*.53,.55,0]);node('UpperArmMesh_'+side,arm,Q.cyl_skin,[0,-.34,0],[.14,.72,.14]);const fore=node('ForeArm_'+side,arm,undefined,[0,-.69,0]);node('ForeArmMesh_'+side,fore,Q.cyl_skin2,[0,-.29,0],[.12,.60,.12]);node('Hand_'+side,fore,Q.sphere_skin2,[0,-.62,0],[.15,.16,.13]);const leg=node('UpperLeg_'+side,hips,undefined,[sx*.25,-.30,0]);node('UpperLegMesh_'+side,leg,Q.cyl_skin,[0,-.31,0],[.18,.64,.18]);const shin=node('LowerLeg_'+side,leg,undefined,[0,-.63,0]);node('LowerLegMesh_'+side,shin,Q.cyl_skin2,[0,-.28,0],[.145,.58,.145]);node('Sandal_'+side,shin,Q.sphere_brown,[0,-.61,-.06],[.20,.10,.31]);return {arm,fore,leg,shin};}
